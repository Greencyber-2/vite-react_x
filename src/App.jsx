...
import Profile from "./components/Profile"; // اضافه کردن Profile
import { Menu } from "lucide-react";

const App = () => {
  // Main app state
  const [isLoading, setIsLoading] = useState(false);
  const [isProfileOpen, setIsProfileOpen] = useState(false); // حالت جدید برای پروفایل
  ...
  return (
    <div className={`app-container ${theme === "light" ? "light-theme" : "dark-theme"}`}>
      ...
      <Sidebar 
        ...
        setIsProfileOpen={setIsProfileOpen} // پاس دادن setIsProfileOpen
      />
      <main className="main-container">
        ...
        {currentConversation.messages.length === 0 ? (
          <div className="welcome-container">
            <img className="welcome-logo" src="gemini.png" alt="Gemini Logo" />
            <h1 className="welcome-heading">Message Jinx+</h1>
        ...
        <div className="prompt-container">
          ...
          <p className="disclaimer-text">Jinx can make mistakes, so double-check it.</p>
        </div>
      </main>
      {isProfileOpen && <Profile onClose={() => setIsProfileOpen(false)} />} {/* رندر مشروط Profile */}
    </div>
  );
};
