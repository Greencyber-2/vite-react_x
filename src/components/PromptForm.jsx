...
const PromptForm = ({ conversations, setConversations, activeConversation, generateResponse, isLoading, setIsLoading }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setConversations(conversations.map((conv) => 
      conv.id === activeConversation ? { ...conv, title: newTitle, messages: [...conv.messages, userMessage] } : conv
    ));
    ...
    setTimeout(() => {
      ...
      setConversations((prev) => 
        prev.map((conv) => 
          conv.id === activeConversation ? { ...conv, title: newTitle, messages: [...conv.messages, botMessage] } : conv
        )
      );
    ...
    <div className="prompt-wrapper">
      ...
      <form className="prompt-form" onSubmit={handleSubmit}>
        <input
          ...
          accept="image/*"
          multiple
          // به جای استفاده از display: none، از استایل زیر استفاده می‌کنیم تا عنصر در DOM وجود داشته و قابل کلیک باشه
